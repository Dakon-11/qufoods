...
def read_csv_files():
    branches_df = pd.read_csv(
        BASE_DIR / "../data/reference/reference_branches.csv"
    )

    menu_df = pd.read_csv(
        BASE_DIR / "../data/reference/reference_menu_items.csv"
    )
    ...
    for _, row in sales_df.iterrows():
        ...
        session.execute(
            query,
            {
                ...
                "order_items_typo_fixed": False if pd.isna(row.order_items_typo_fixed) else bool(
                    row.order_items_typo_fixed),
